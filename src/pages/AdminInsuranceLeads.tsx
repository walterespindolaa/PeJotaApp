...
import { Shield, Star, Phone, Mail, Calendar } from "lucide-react";
...
const AdminInsuranceLeads = () => {
  ...
  const fetchLeads = useCallback(async () => {
    ...
    else setLeads((data as InsuranceLead[]) || []);
    setLoading(false);
  }, [toast]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h2 className="font-heading font-bold text-base">Leads de Seguro</h2>
        ...
        <CardContent className="p-0">
          {leads.length === 0 ? (
            <div className="p-8 text-center text-sm text-muted-foreground">
              Nenhum lead de seguro recebido ainda.
            </div>
          ) : (
              ...
              <Table>
                ...
                <TableBody>
                  {leads.map(lead => {
                    ...
                    return (
                      <TableRow key={lead.id} className="cursor-pointer hover:bg-muted/30" onClick={() => { setSelectedLead(lead); setDrawerOpen(true); }}>
                        <TableCell className="text-xs whitespace-nowrap">{new Date(lead.created_at).toLocaleDateString("pt-BR")}</TableCell>
                        <TableCell className="text-xs font-medium">{lead.nome}</TableCell>
                        <TableCell className="text-xs">{lead.dependentes}</TableCell>
                        <TableCell className="text-xs">{lead.patrimonio_investido}</TableCell>
          ...
          <>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  { label: "Dependentes", value: String(selectedLead.dependentes) },
                  { label: "Score", value: scoreStars(selectedLead.score) },
                  { label: "Patrimônio investido", value: selectedLead.patrimonio_investido },
                  { label: "Patrimônio imobiliário", value: selectedLead.patrimonio_imobiliario },
                  { label: "Renda mensal", value: selectedLead.renda_mensal },
                  { label: "Herança esperada", value: selectedLead.heranca_esperada },
                  { label: "Seguro atual", value: selectedLead.seguro_atual },
                ].map((item, i) => (
...
export default AdminInsuranceLeads;
