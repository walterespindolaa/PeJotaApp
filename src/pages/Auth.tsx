...
const Auth = () => {
  ...
  return (
    <>
      <SEO
        title="Entrar no PeJota | Acesse sua conta"
        description="Acesse o PeJota para gerir o caixa, vendas, propostas e estoque do seu negócio num só lugar."
        path="/auth"
      />
      <div className="min-h-screen relative overflow-hidden">
        {/* Fundo animado (shader) na paleta PeJota */}
        <ShaderBackground className="absolute inset-0 w-full h-full" />
        {/* Escurecimento à esquerda (desktop) para o texto da marca ficar legível */}
        <div className="hidden lg:block absolute inset-y-0 left-0 w-3/5 pointer-events-none" style={{ background: "linear-gradient(to right, rgba(12,22,32,0.78), rgba(12,22,32,0.35), transparent)" }} aria-hidden="true" />
        ...
        <div className="relative z-10 min-h-screen flex items-end justify-center px-6 pb-10 lg:items-center lg:pb-0">
          <div className="w-full max-w-5xl grid lg:grid-cols-2 gap-10 lg:gap-12 items-center animate-fade-in">
            ...
            <div className="hidden lg:flex flex-col text-white bg-black/35 backdrop-blur-md rounded-3xl p-9 border border-white/10">
              <span className="inline-flex items-center gap-2 self-start px-[14px] py-[6px] rounded-full bg-[#2f8bff]/18 border border-[#2f8bff]/40 text-[#2f8bff] text-xs font-semibold tracking-wide mb-6">
                <Zap className="h-3.5 w-3.5" /> Gestão financeira para PJ
              </span>
              <h1 className="font-black leading-[1.04] tracking-tight" style={{ fontSize: "clamp(40px, 4.2vw, 58px)", textShadow: "0 2px 24px rgba(0,0,0,0.35)" }}>
                O caixa da sua empresa,<br />sob controle.
              </h1>
              <p className="mt-5 text-[15px] max-w-[380px]" style={{ color: "rgba(255,255,255,0.85)", textShadow: "0 1px 12px rgba(0,0,0,0.35)" }}>
                Fluxo de caixa, contas a pagar e receber, vendas, propostas e estoque — tudo num só lugar.
              </p>
              <div className="mt-7 flex flex-col gap-3">
                {["Visão clara do caixa em tempo real", "Funil de vendas e propostas", "Dados isolados e seguros por empresa"].map((b) => (
                  <div key={b} className="flex items-center gap-2.5 text-[14px]" style={{ color: "rgba(255,255,255,0.9)", textShadow: "0 1px 10px rgba(0,0,0,0.4)" }}>
                    <span className="h-1.5 w-1.5 rounded-full bg-[#2f8bff] flex-shrink-0" />
                    {b}
                  </div>
                ))}
              </div>
            ...
            <div className="w-full max-w-sm mx-auto lg:ml-auto">
              ...
              >
                ...
                <span className="lg:hidden inline-flex items-center gap-2 px-[13px] py-[5px] rounded-full bg-[#2f8bff]/15 border border-[#2f8bff]/35 text-[#1a5e91] text-[11px] font-semibold tracking-wide mb-4">
                  <Zap className="h-3 w-3" /> Gestão financeira para PJ
                </span>
                ...
                <form onSubmit={handleLogin} className="space-y-5">
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      className="border-[#E0D9CC] bg-white focus-visible:ring-[#2f8bff]/50 placeholder:text-[#C4B9A4] rounded-xl h-12 md:h-11 text-base md:text-sm"
                    />
                  </div>
                  <div className="space-y-1.5">
                  ...
                  <Button
                    ...
                    style={{ background: "#2f8bff", color: "#0c1620", boxShadow: "0 4px 18px rgba(242,171,12,0.28)" }}
                    disabled={loading}
                  >
