...
// O ID vem da env VITE_META_PIXEL_ID (configurada no Lovable). Sem ID, tudo vira no-op.
// Os event_id gerados aqui permitem deduplicar com o CAPI server-side (passo 2).
...
/** Dispara um evento padrão do Pixel (PageView, Purchase, InitiateCheckout, etc.). */
export function track(event: string, params?: Record<string, any>, eventId?: string): void {
